# Story 1 — Current Scheduling Flow Analysis & Baseline Mapping

- [x] ~~1.1 \- Federico C.~~  
- [x] ~~1.2 \- Luca~~   
- [x] ~~1.3 \- Federico C.~~  
- [x] ~~1.4 Luca~~  
- [x] ~~1.5 Andrea~~  
- [x] ~~1.6 Federico S.~~ 

\---

# Story 2 — AI Agent Communication & TOON/JSON Protocol Design (Direct Prompting)

- [x] ~~2.1 Luca~~  
- [x] ~~2.2 Andrea~~  
- [x] ~~2.3 Federico S.~~  
- [x] ~~2.4 Giuseppe~~  
- [x] ~~2.5 Luca~~  
- [x] ~~2.6 Luca~~  
- [x] ~~2.7 Federico C.~~  
- [x] ~~2.8 Cantone~~

\---

# Story 3 — Metrics & Decision Framework (GQM+Strategy Integration)

- [x] ~~3.1 Andrea e Federico S.~~  
- [x] ~~3.2 Andrea e Federico S.~~  
- [x] ~~3.3 Andrea e Federico S. Cantone, Capone~~
- [x] ~~3.4  Cantone, Capone, Scordo~~
- [x] ~~3.5 Capone~~
- [x] ~~3.6 Capone~~
- [x] ~~3.7 Federico C.~~

\---

# Story 4 — Planner UI: Schedule Comparison & Selection

- [x] ~~4.1 Scordo~~
- [ ] 4.2 Andrea
- [ ] 4.3  DEPENDENCIES 4.2
- [x] ~~4.4 Scordo~~
- [x] ~~4.5 Scordo~~

\---

# Story 5 — Backend Orchestration Plan for AI Rescheduling

- [x] ~~5.1 Cantone~~
- [x] ~~5.2 Cantone~~
- [x] ~~5.3 Scordo~~
- [x] ~~5.4  Capone~~
- [x] ~~5.5  Cantone~~
- [x] ~~5.6 Federico C.~~

\---

# Story 6 — Documentation & Stakeholder Materials

- [x] ~~6.1~~  
  - [x] ~~6.1.a Capture backend orchestration flow diagram draft~~
  - [x] ~~6.1.b Draft AI protocol section (TOON/JSON + retries/timeouts)~~
  - [x] ~~6.1.c Draft metrics + decision algorithm section~~
  - [x] ~~6.1.d Draft UI comparison/selection flow notes~~
  - [x] ~~6.1.e Assemble glossary + data minimization notes~~
- [x] ~~6.2~~ 
  - [x] ~~6.2.a Write sprint timeline summary~~
  - [x] ~~6.2.b Capture decisions log~~
  - [x] ~~6.2.c Record open questions/risks + mitigations~~
  - [x] ~~6.2.d Add dependency impacts for remaining work~~
- [x] ~~6.3~~ 
  - [x] ~~6.3.a Draft slides 1–2 (problem + baseline flow)~~
  - [x] ~~6.3.b Draft slides 3–4 (AI approach + orchestration)~~
  - [x] ~~6.3.c Draft slide 5 (metrics + decision framework)~~
  - [x] ~~6.3.d Draft slide 6 (UI preview + next steps)~~
- [ ] 6.4
  - [ ] 6.4.a Collect “what went well” bullets
  - [ ] 6.4.b Collect “what didn’t go well” bullets
  - [ ] 6.4.c Capture risks + mitigations for next sprint
  - [ ] 6.4.d Note improvement actions

\---

\---

## **Effort Summary**

* Story 1: 12h  
* Story 2: 16h  
* Story 3: 18h  
* Story 4: 10h  
* Story 5: 10h  
* Story 6: 4h  
* Total: 70 hours
