...
### Software objects and responsibilities
| Exact path | Primary identifier | Responsibility | Key interactions |
| --- | --- | --- | --- |
| `src/main/java/org/cswteams/ms3/rest/ScheduleFeedbackRestEndpoint.java` | `ScheduleFeedbackRestEndpoint` | Exposes doctor/planner feedback APIs and delegates all persistence rules to the schedule-feedback controller layer. | Called by planner/doctor UI; calls `IScheduleFeedbackController` methods. |
| `frontend/src/views/pianificatore/FeedbackManagementView.js` | `FeedbackManagementView` | Renders the planner feedback table with sorting and modal drill-down for text comments. | Instantiates `ScheduleFeedbackAPI`; called by planner navigation flow. |
| `src/main/java/org/cswteams/ms3/rest/ScheduleRestEndpoint.java` | `createScheduleWithAi(...)` | Starts AI-assisted comparison generation for a date range and maps result availability to HTTP responses. | Called by `ScheduleGeneratorView`; calls `AiScheduleGenerationOrchestrationService.generateScheduleComparison(...)`. |
| `frontend/src/views/pianificatore/ScheduleGeneratorView.js` | `ScheduleGeneratorView` | Orchestrates planner-side generation/regeneration actions and opens AI comparison/selection UX after backend responses. | Calls schedule/comparison APIs; renders `GenerationStatusFeedback`, `AiScheduleComparisonModal`, and `AiScheduleSelectionConfirmationModal`. |
...
| Exact path | Primary identifier | Responsibility | Key interactions |
| --- | --- | --- | --- |
| `src/main/java/org/cswteams/ms3/ai/broker/AiBrokerConfiguration.java` | `AiBrokerConfiguration` | Wires the selected AI provider adapter and broker-related beans from configuration properties. | Creates `AgentBrokerImpl`; injects provider adapters and protocol parser. |
| `src/main/java/org/cswteams/ms3/ai/broker/AgentBrokerImpl.java` | `AgentBrokerImpl` | Executes provider calls with timeout/retry/token-budget protections and normalizes transport/protocol failures. | Called by orchestration services; calls `AgentProviderAdapter`, `AiScheduleJsonParser`, and `AiScheduleResponseMapper`. |
| `src/main/java/org/cswteams/ms3/ai/orchestration/AiScheduleGenerationOrchestrationService.java` | `AiScheduleGenerationOrchestrationService` | Coordinates baseline creation, AI request/validation, metric normalization, and candidate comparison caching/selection. | Called by schedule/comparison REST endpoints; calls `AgentBroker`, decision service, scheduler controller, DAOs, and mappers. |
| `src/main/java/org/cswteams/ms3/rest/ComparisonRestEndpoint.java` | `ComparisonRestEndpoint` | Provides planner endpoints to fetch the latest comparison and persist the final candidate selection outcome. | Called by planner comparison UI; calls `AiScheduleGenerationOrchestrationService` query/selection methods. |
...
| Exact path | Primary identifier | Responsibility | Key interactions |
| --- | --- | --- | --- |
| `src/main/java/org/cswteams/ms3/ai/protocol/AiScheduleJsonParser.java` | `AiScheduleJsonParser` | Parses and schema-checks AI JSON payloads (single/variants) and raises typed protocol exceptions on contract mismatches. | Called by `AgentBrokerImpl`; returns protocol DTOs or throws `AiProtocolException`. |
| `src/main/java/org/cswteams/ms3/ai/protocol/AiScheduleSemanticValidator.java` | `AiScheduleSemanticValidator` | Enforces semantic constraints on parsed AI payloads beyond JSON syntax/schema validity. | Called during AI candidate validation in orchestration flow; consumes protocol DTO fields. |
| `src/main/java/org/cswteams/ms3/ai/broker/mapper/AiScheduleResponseMapper.java` | `AiScheduleResponseMapper` | Maps protocol DTO structures into broker domain objects used by scheduling orchestration. | Called by `AgentBrokerImpl`; transforms `AiScheduleResponseDto` to `AiScheduleResponse`. |
| `src/main/java/org/cswteams/ms3/ai/protocol/dto/AiScheduleVariantsResponseDto.java` | `AiScheduleVariantsResponseDto` | Represents the versioned payload shape containing labeled AI variants for downstream selection logic. | Produced by `AiScheduleJsonParser`; consumed by broker/orchestration validation and mapping steps. |
...
| Exact path | Primary identifier | Responsibility | Key interactions |
| --- | --- | --- | --- |
| `src/main/java/org/cswteams/ms3/rest/ScheduleFeedbackRestEndpoint.java` | `updateFeedback(...)` / `deleteFeedback(...)` | Enforces ownership-scoped update/delete APIs so only the doctor owner can alter feedback before planner review. | Called by doctor UI flows; calls `IScheduleFeedbackController` with authenticated email. |
| `frontend/src/views/pianificatore/FeedbackManagementView.js` | `componentDidMount()` + table sort handlers | Retrieves planner-visible feedback and supports quick triage through sortable fields and per-item text inspection. | Calls `ScheduleFeedbackAPI.getFeedbacks()`; used by planner moderation workflow. |
| `src/main/java/org/cswteams/ms3/ai/orchestration/AiActiveConstraintResolver.java` | `AiActiveConstraintResolver` | Resolves active constraints that define what feedback/proposal outcomes are acceptable for the AI run context. | Called by orchestration services while preparing request and validation context. |
| `src/main/java/org/cswteams/ms3/rest/PreferencesRestEndpoint.java` | `PreferencesRestEndpoint` | Maintains doctor preference signals that can complement explicit schedule feedback during acceptability decisions. | Called by user preference UI; delegates to preference controller/services. |
...
| Exact path | Primary identifier | Responsibility | Key interactions |
| --- | --- | --- | --- |
| `src/main/java/org/cswteams/ms3/ai/orchestration/AiScheduleGenerationOrchestrationService.java` | `selectSchedule(...)` and candidate validation flow | Re-validates and gates AI candidates before persistence, ensuring hard constraints and selection invariants are respected. | Called by `ScheduleRestEndpoint`/`ComparisonRestEndpoint`; calls scheduler/constraint logic and decision utilities. |
| `src/main/java/org/cswteams/ms3/security/SecurityConfigurer.java` | `configure(HttpSecurity)` | Defines authenticated boundaries for login/logout and secures all remaining endpoints with JWT-based stateless rules. | Called by Spring Security bootstrap; positions `JwtRequestFilters` before auth filters. |
| `src/main/java/org/cswteams/ms3/control/logout/LogoutController.java` | `LogoutController.logout(...)` | Resolves authenticated user identity and blacklists the presented JWT as part of governance for session termination. | Called by `LogoutRestEndpoint`; calls `SystemUserDAO` and `JwtBlacklistService`. |
| `src/main/java/org/cswteams/ms3/control/logout/JwtBlacklistService.java` | `JwtBlacklistService` | Persists token invalidation markers (single token and wildcard) and exposes blacklist checks used by request filtering. | Called by logout and auth-filter flows; calls `BlacklistedTokenDAO` and `JwtUtil`. |
...
| Exact path | Primary identifier | Responsibility | Key interactions |
| --- | --- | --- | --- |
| `frontend/src/views/pianificatore/ScheduleGeneratorView.js` | `ScheduleGeneratorView` | Hosts planner UX for generation actions and AI candidate decision steps in one operational screen. | Calls schedule/comparison backend endpoints; opens comparison and confirmation modals. |
| `frontend/src/components/common/AiScheduleComparisonModal.js` | `AiScheduleComparisonModal` | Displays baseline and AI candidates side-by-side with key metrics and selection controls. | Called by `ScheduleGeneratorView`; invokes parent callbacks for candidate selection. |
| `frontend/src/components/common/AiScheduleSelectionConfirmationModal.js` | `AiScheduleSelectionConfirmationModal` | Enforces explicit user confirmation before finalizing irreversible schedule selection. | Called by `ScheduleGeneratorView`; calls `onConfirm`/`onCancel` handlers. |
| `frontend/src/views/pianificatore/FeedbackManagementView.js` | `FeedbackManagementView` | Supports pre-request moderation by giving planners a navigable view of feedback quality and details. | Called from planner routes; consumes `ScheduleFeedbackAPI` data. |
...
| Exact path | Primary identifier | Responsibility | Key interactions |
| --- | --- | --- | --- |
| `src/main/java/org/cswteams/ms3/rest/ScheduleRestEndpoint.java` | `createSchedule(...)` and logging helpers | Emits generation/regeneration lifecycle events with timing/result metadata for operational diagnostics. | Called by planner UI; calls scheduler controller and orchestration service. |
| `src/main/java/org/cswteams/ms3/ai/broker/AgentBrokerImpl.java` | `requestSchedule(...)` | Logs provider-level request metadata and enforces retry/token policies that feed runtime monitoring signals. | Called by orchestration; calls adapters and token usage tracker. |
| `src/main/java/org/cswteams/ms3/control/logout/ExpiredTokensRemovalService.java` | `deleteExpiredTokens()` | Runs scheduled cleanup of expired blacklist entries to keep token-governance storage healthy. | Called by Spring scheduler; calls `BlacklistedTokenDAO.deleteByExpiresAtBefore(...)`. |
| `frontend/src/components/common/GenerationStatusFeedback.js` | `GenerationStatusFeedback` | Surfaces success/partial/error outcomes in UI so planners immediately see operational state and warnings. | Called by `ScheduleGeneratorView`; invokes parent close callback to reset status state. |
