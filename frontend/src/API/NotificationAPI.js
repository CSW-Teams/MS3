import {fetchWithAuth} from "../utils/fetchWithAuth";

export class NotificationAPI {

  async getNotification(id) {
    try {
      const response = await fetchWithAuth(`/api/notification/id=${encodeURIComponent(id)}`);
      if (response.status === 200) {
        const body = await response.json();
        const notifications = [];
        for (let i = 0; i < body.length; i++) {
          const notification = {};
          notification.idNotification = body[i].idNotification;
          notification.idUser = body[i].idUser;
          notification.description = body[i].description;
          notifications[i] = notification;
        }
        return notifications;
      }
    } catch (error) {
      console.error('Errore durante il recupero delle notifiche:', error);
    }
    return [];
  }

  async removeNotification(id, description, idUser) {
    try {
      const data = {
        idNotification: Number(id),
        description,
        idUser: Number(idUser),
      };
      const response = await fetchWithAuth('/api/notification/updateStatus', {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(data),
      });
      if (!response.ok) {
        throw new Error(`Error with the HTTP response: ${response.status}`);
      }
    } catch (error) {
      console.error('Errore durante il recupero delle notifiche:', error);
    }
  }
}
