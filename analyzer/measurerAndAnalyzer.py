...
def computazionePerSchedule(name):
    ...
    dictFindMin={}
    dictFinDiffMin={}
    keys=["giornaliero","pomeridiano","notturno","domeniche","lunga","uffaPoint"]
    for schedule_id in AllSchedule["schedule_id"]:
        ...
        for ms3_id in AllUser["ms3_system_user_id"]:
            ...
            queryUffa = "SELECT uffa_cumulativo FROM doctor_schedule_state WHERE schedule_schedule_id="+str(schedule_id)+" and doctor_ms3_system_user_id="+str(ms3_id)
            count5 = pd.read_sql(queryUffa,con=conn)
            dictApp[ms3_id] = {
                "giornaliero":count0["c"].values[0],
                "pomeridiano":count1["c"].values[0],
                "notturno":count2["c"].values[0],
                "domeniche":count3["c"].values[0],
                "lunga":count4["c"].values[0],
                "uffaPoint":count5["uffa_cumulativo"].values[0]
            }
