...
public class ConstraintMaxPeriodoConsecutivo extends ConstraintAssegnazioneTurnoTurno {
    ...
    public void verifyConstraint(ContextConstraint context) throws ViolatedConstraintException {
        if(context.getDoctorXY().getAssegnazioniTurnoCache().size() != 0 && verificaAppartenenzaCategoria(context)) {
            List<ConcreteShift> assignedConcreteShifts = context.getDoctorXY().getAssegnazioniTurnoCache();
            ...
            if (consecMinutes > maxConsecutiveMinutes) {
                throw new ViolatedVincoloAssegnazioneTurnoTurnoException(context.getConcreteShift(), context.getDoctorXY().getDoctor(), maxConsecutiveMinutes);
            }
        }
    ...
    private boolean verificaAppartenenzaCategoria(ContextConstraint context){
        ...
        if(constrainedCategory == null){
         ...
         for (Condition condition : context.getDoctorXY().getDoctor().getPermanentConditions()) {
